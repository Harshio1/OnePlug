                                          ...
                                          <td className="py-4 px-6 text-white truncate max-w-xs">
                                            <div className="flex items-center gap-3">
                                              ...
                                              <div className="truncate flex flex-col gap-1">
                                                <span className="truncate text-base font-semibold">{file.caller_number || `Call ${sortedFiles.length - fileIndex}`}</span>
                                                {file.customers && file.customers.length > 0 && file.customers[0].customer_name && (
                                                  <span className="truncate text-base font-bold text-white leading-tight">Customer Name: {file.customers[0].customer_name}</span>
                                                )}
                                                {file.agent_name && (<span className="truncate text-sm text-brand-text-muted">Agent Name: {file.agent_name}</span>)}
                                                {file.call_direction && (<span className={`truncate text-sm font-semibold ${file.call_direction === "inbound" ? "text-brand-green" : "text-blue-400"}`}>{file.call_direction === "inbound" ? "Inbound" : "Outbound"}</span>)}
                                                {file.customers && file.customers.length > 0 && (
                                                  <div className="mt-1 flex flex-col gap-0.5">
                                                    {file.customers.map((c: any, ci: number) => (
                                                      <div key={ci} className="text-xs text-brand-text-muted border-t border-brand-border/20 pt-1 flex flex-col gap-0.5">
                                                        {c.station_name && <span>Station Name: <span className="text-white">{c.station_name}</span>{c.location && <span className="text-brand-text-muted/70"> ({c.location})</span>}</span>}
                                                        {c.vehicle_modal && <span>Vehicle Model: <span className="text-brand-green">{c.vehicle_modal}</span></span>}
                                                      </div>
                                                    ))}
                                                  </div>
                                                )}
